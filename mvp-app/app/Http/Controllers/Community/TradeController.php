    ...
    public function storeTrade(Request $request)
{
    ...
    return redirect()->route('community.index', ['active_tab' => $request->get('active_tab', 'trade')])
    ->with('success', '投稿しました!');
}

public function updateTrade(Request $request, $id)
{
    $tradePosts = TradePost::findOrFail($id);

    if($tradePosts->user_id !== Auth::id()){
...
public function destroyTrade(Request $request,$id)
 {
 ...
 public function storeTradeComment(Request $request, $postId)
 {
...
public function updateTradeComment(Request $request, $commentId)
{
...
public function destroyTradeComment(Request $request,$commentId)
{
    $comment = TradePostComment::findOrFail($commentId);
