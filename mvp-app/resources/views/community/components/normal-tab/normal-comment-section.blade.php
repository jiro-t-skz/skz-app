...
<div class="mt-4 pt-4 border-t-2 border-sakura-pink/30" x-data="{ showComments: false }">
    ...
    <button
        ...
        class="text-sm sm:tex-base
            ...
            <svg class="w-4 h-4 sm:w-5 sm:h-5" fill="currentColor" viewBox="0 0 20 20">
             ...
             コメント ({{ $normalPost->comments->count() }})
            <svg
    ...
    <div x-show="showComments"
        ...
        @forelse($normalPost->comments as $comment)
            <div class="bg-gradient-to-br from-sakura-pink/5 to-white
                            ...
                            <div class="flex gap-1.5 flex-shrink-0">
                                <button
                                    ...
                                    class="px-2 py-1
                                           ...
                                           transition-all cduration-200">
                                    編集
                                </button>
                                <form method="POST"
                                      action="{{ route('community.normal.comments.destroy', $comment->id) }}">
                                    @csrf
                                    @method('DELETE')
                                    <button
                                        type="submit"
                                        ...
                                        class="px-2 py-1
                                               ...
                                               text-red-400
                                               rounded-md text-xs font-medium
                                               focus:outline-none focus:ring-2 focus:ring-2 focus:ring-red-300
                ...
                <div x-show="editingComment"
                    ...
                    <form method="POST" action="{{route('community.normal.comments.update', $comment->id)}}">
                        @csrf
                        @method('PUT')
                        <textarea
                            name="body"
        ...
        @endforelse
        {{-- 新規コメント投稿フォーム --}}
        <form method="POST" action="{{ route('community.normal.comments.store', $normalPost->id) }}" class="mt-4">
            @csrf
