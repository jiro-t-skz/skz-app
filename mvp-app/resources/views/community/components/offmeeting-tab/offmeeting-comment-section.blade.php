...
{{-- コメントセクション--}}
<div class="mt-4 pt-4 border-t-2 border-sakura-pink/30" x-data="{ showComments: false }" x-cloak>
    {{--コメント数表示とトグル--}}
    <button
        ...
        class="text-sm sm:text-base
            ...
            <svg class="w-4 h-4 sm:w-5 sm:h-5" fill="currentColor" viewBox="0 0 20 20">
             ...
             コメント ({{ $offmeetingPost->comments->count() }})
            <svg
    ...
    <div x-show="showComments"
        ...
        @forelse($offmeetingPost->comments as $comment)
            <div class="bg-gradient-to-br from-sakura-pink/5 to-white
                    ...
                    <div class="flex items-start justify-between mb-2 gap-2">
                        ...
                        @if($comment->user_id === Auth::id())
                            <div class="flex gap-1.5 flex-shrink-0">
                                ...
                                <form method="POST"
                                      action="{{ route('community.offmeeting.comments.destroy', $comment->id) }}">
                                    @csrf
                                    @method('DELETE')
                                    <button
                                        type="submit"
                ...
                <div x-show="editingComment"
                    ...
                    <form method="POST" action="{{route('community.offmeeting.comments.update', $comment->id)}}">
                        @csrf
                        @method('PUT')
                        <textarea
                            name="body"
                            ...
                            class="w-full px-3 py-2
                                   ...
                                   resize-none
                                   transition-all duration-300">{{ $comment->body }}</textarea>
        ...
        @endforelse
        {{-- 新規コメント投稿フォーム --}}
        <form method="POST" action="{{ route('community.offmeeting.comments.store', $offmeetingPost->id) }}" class="mt-4">
            @csrf
